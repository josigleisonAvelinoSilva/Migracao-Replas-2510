...
ฑฑบPrograma  ณPROCESS4  บAutor  ณ      บ Data ณ                           บฑฑ
...
User Function Process4(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
oObj := MsNewProcess():New({|lEnd| BOLBRASI(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo) },"Processando","Gerando Boletos...",.T.)	//Processamento da gera็ใo de boletos
oObj:Activate()	

Return
 
/*ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
ฑฑบ Programa      ณ BOLBRASI                         บ Data ณ             บฑฑ
ฑฑฬอออออออออออออออุออออออออออออออออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบ Descricao     ณ Programa para Geracao de Boleto Grafico               บฑฑ
ฑฑบ				  ณ	utilizando o Objeto FWMSPTRINTER.					  บฑฑ
ฑฑฬอออออออออออออออุออออออออออออออออออออออหอออออออออัออออออออออออออออออออออนฑฑ
ฑฑบ Desenvolvedor ณ                      บ Empresa ณ                      บฑฑ
ฑฑบ Alterado por  ณ                      บ Empresa ณ                      บฑฑ
ฑฑฬอออออออออออออออุออออออออออออหออออออออัสออออออหออฯออออออออออออออออออออออนฑฑ
ฑฑบ Linguagem     ณ Advpl      บ Versao ณ       บ Sistema ณ Microsiga     บฑฑ
...
ฑฑฬอออออออออออออออุอออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
ฑฑบ Observacao    ณ                                                       บฑฑ
฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿฿*/

Static Function BOLBRASI(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
If !ExistDir("C:\Boleto001\")
	MontaDir("C:\Boleto001\")
EndIf    
...
For i := 1 to Len(aVetor)
	...
	If aVetor[i,1] == .T.
		...
		_nJuros		:= SEE->EE_XJUROS                                                 	
		_nMulta		:= SEE->EE_XMULTA
		_cProtesto	:= SEE->EE_DIASPRT
		_cCodEmp	:= SEE->EE_CODEMP
		
		aAdd(_aBoletos,{SE1->(Recno()), SE1->E1_NUM, SE1->E1_TIPO, SC5->(Recno()), SC5->C5_NUM, "",'',Ctod(''),''})
		
		lRet	:=	U_CalcBras(_aBoletos,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
		...
		_cArquivo := AllTrim(SE1->E1_FILIAL) + "_" + AllTrim(SE1->E1_NUM) + "_" + Alltrim(SE1->E1_PARCELA) + "_001 " + ".pdf"
		cFileName := "C:\Boleto001\" + _cArquivo
		// Impressao
		...
		If _cBanco == "001"
			oPrint:Say(0056, 0331,Substr(Alltrim(_cAgencia),1,4)+"-"+Alltrim(_cDvAge)+"/"+Substr(Alltrim(_cConta),1,6)+"-"+Alltrim(_cDvCta),oFont06,100)			
		EndIf
		...
		If _cBanco == "001"
			...
			oPrint:Say  (0200, 0096,"APำS, ATUALIZE O BOLETO NO SITE BB.COM.BR.",oFont06N )
		EndIf
		...
		If _cBanco=="001"
			oPrint:Say(0221, 0436,Substr(Alltrim(_cAgencia),1,4)+"-"+Alltrim(_cDvAge)+"/"+Substr(Alltrim(_cConta),1,6)+"-"+Alltrim(_cDvCta),oFont06,100)
		EndIf
		...
		If _cBanco=="001"
			oPrint:Say (0246, 0476, Alltrim(_cCodEmp) + Substr(SE1->E1_NUMBCO,6,5),oFont06 )
		EndIf
		...
		If SE1->(FieldPos("E1_JUROS")) > 0
			If !Empty(SE1->E1_JUROS)
				oPrint:Say  (0306,0026,"Juros mora de R$ " + Alltrim(Transform(SE1->E1_JUROS,"@E 99,999,999.99"))+ " de "+DtoC(SE1->E1_VENCTO)+" a "+DtoC(SE1->E1_VENCREA), oFont08,100)
			EndIf  
		EndIf
		If SE1->(FieldPos("E1_XMULTA")) > 0	
			If !Empty(SE1->E1_XMULTA)	
				oPrint:Say  (0316,0026,"Multa por atraso do pagamento de R$ " + Alltrim(Transform(SE1->E1_XMULTA,"@E 99,999,999.99")), oFont08,100)
		    EndIf
		Endif
		oPrint:Say  (0326,0026,"Sr. Caixa nใo receber apos vencimento. ", oFont08,100)
		...
		If _cBanco = "001"
			...
			oPrint:Say  (0520, 0096,"APำS, ATUALIZE O BOLETO NO SITE BB.COM.BR.",oFont06N )
		EndIf
		...
		If _cBanco=="001"
			oPrint:Say(0541, 0436,Substr(Alltrim(_cAgencia),1,4)+"-"+Alltrim(_cDvAge)+"/"+Substr(Alltrim(_cConta),1,6)+"-"+Alltrim(_cDvCta),oFont06,100)
		EndIf
		...
		If _cBanco=="001"
			oPrint:Say (0566, 0476,Alltrim(_cCodEmp) + Substr(SE1->E1_NUMBCO,6,5),oFont06 )
		EndIf
		...
		If _cBanco == "001"
			oPrint:Say (0591, 0131,Substr(_cCart,1,2),oFont06 )
		EndIf
		...
		If SE1->(FieldPos("E1_JUROS")) > 0
			If !Empty(SE1->E1_JUROS)
				oPrint:Say  (0626,0026,"Juros mora de R$ " + Alltrim(Transform(SE1->E1_JUROS,"@E 99,999,999.99"))+ " de "+DtoC(SE1->E1_VENCTO)+" a "+DtoC(SE1->E1_VENCREA), oFont08,100)
			EndIf
		EndIf
		If SE1->(FieldPos("E1_XMULTA")) > 0	
			If !Empty(SE1->E1_XMULTA)	
				oPrint:Say  (0636,0026,"Multa por atraso do pagamento de R$ " + Alltrim(Transform(SE1->E1_XMULTA,"@E 99,999,999.99")), oFont08,100)
		    EndIf
		Endif
		oPrint:Say  (0646,0026,"Sr. Caixa nใo receber apos vencimento. ", oFont08,100)
...
MsgInfo("Foram Salvos "+AllTrim(Str(nCont))+" Boletos no Diret๓rio C:\Boleto001\")
//Close(oDlg)
WinExec( "Explorer.exe C:\Boleto001"  )
Return
...
/*ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
ฑฑบPrograma  ณRetImp   บAutor  ณ      บ Data ณ                            บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
...
/*ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
ฑฑบPrograma  ณMTBCO     บ Autor ณ     บ Data ณ                            บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
...
User Function CalcBras(_aBoletos,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
For _nxx:=1 To Len(_aBoletos)
	If Empty(_aBoletos[_nxx][6])
		...
		lRet	:=	U_CodBco001(_aBoletos[_nxx,2],_aBoletos[_nxx,3],_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
User Function CodBco001(_cNumeIni,cInull,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
Else
	Do Case
		Case _cBanco == "001"
			_cNossoNum	:= Nosso001(_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
	EndCase
EndIf
...
Do Case
	Case _cBanco == "001" //Banco Do Brasil
		...
		_cCampo4 := _cDigBar
		_cCampo5 := Substr(_cNumBar,06,4) + Strzero((IIf(SE1->E1_PREFIXO<>"RPS",SE1->E1_SALDO+SE1->E1_ACRESC,(SE1->E1_SALDO+SE1->E1_ACRESC)- (aValImps[5] + aValImps[3]+aValImps[4]+aValImps[7]+aValImps[2] + aValImps[6])))*100,10)
...
Do Case
	Case _cBanco == "001" // Banco do Brasil
		...
		EndIf
		_cCampo1 := _cBanco + "9" + _cDigBar + Strzero(nFatVenc,4) + Strzero((IIf(SE1->E1_PREFIXO<>"RPS",SE1->E1_SALDO+SE1->E1_ACRESC,(SE1->E1_SALDO+SE1->E1_ACRESC)- (aValImps[5] + aValImps[3]+aValImps[4]+aValImps[7]+aValImps[2] + aValImps[6])))*100,10) + Substr(SEE->EE_CODEMP,1,6) + Substr(_cNossoNum,6,5) + Strzero(Val(Alltrim(_cAgencia)),4) + Strzero(Val(Alltrim(_cConta)),8) + Substr(SEE->EE_CODCART,2,2)
...
Static Function Nosso001(_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
