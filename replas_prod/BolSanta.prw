...
User Function Process6(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
Default  _cTitulo	:= ""
oObj := MsNewProcess():New({|lEnd| BolSanta(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo) },"Processando","Gerando Boletos...",.T.)	//Processamento da gera็ใo de boletos
oObj:Activate()	
...
Static Function BolSanta(aVetor,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
If !ExistDir("C:\Boleto033\")
	MontaDir("C:\Boleto033\")
EndIf    
nReq := 0
nReq := Len(aVetor)
If oObj != Nil
...
For i := 1 to Len(aVetor)
	...
	If aVetor[i,1] == .T.
		...
		lRet	:=	U_CalcSant(_aBoletos,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
		// Caso de nosso numero nใo preenchido aborta impressใo 
		If !lRet
			Return
		EndIf	
		_cArquivo := AllTrim(SE1->E1_FILIAL) + "_" + AllTrim(SE1->E1_NUM) + "_" + Alltrim(SE1->E1_PARCELA) + "_033" + ".pdf"
		cFileName := "C:\Boleto033\" + _cArquivo
		...
		If _cSubcta == "RVL"
			oPrint:Say (0056, 0026,Alltrim(Substr(SEE->EE_XNOMEVL,1,30)),oFont12 )	// "VIDEOLAR INNOVA S/A"
		Else
		...
		If _cBanco == "033"
			oPrint:Say(0056,0331,Substr(Alltrim(_cAgencia),1,4) + " / " + Iif(_cSubCta == "RVL",Substr(Alltrim(_cCodEmp),9,7),Substr(Alltrim(_cCodEmp),6,7)),oFont12,100)
		EndIf
		...
		oPrint:Say (0081, 0301,Substr( DtoS(SE1->E1_VENCTO),7,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),5,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),1,4 ),oFont12 )
		...
		oPrint:Say (0196, 0476,Substr( DtoS(SE1->E1_VENCTO),7,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),5,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),1,4 ),oFont12 )
		BuzzBox  (0205,0025,0230,0425)	// Beneficiario
		oPrint:Say (0211, 0026,"Beneficiแrio",oFont06N )
		If _cSubcta == "RVL"
			oPrint:Say (0221, 0026,Alltrim(Substr(SEE->EE_XNOMEVL,1,30)),oFont12 )	// "VIDEOLAR INNOVA S/A"
		Else
			...
			oPrint:Say (0221, 0026,Alltrim(Substr(SM0->M0_NOMECOM,1,30)),oFont12 )
		EndIf
		If _cBanco == "033"
		...
		If _cBanco == "033"
			oPrint:Say(0221,0436,Substr(Alltrim(_cAgencia),1,4)+"/"+Iif(_cSubCta == "RVL",Substr(Alltrim(_cCodEmp),9,7),Substr(Alltrim(_cCodEmp),6,7)),oFont12,100)
		EndIf
		...
		If _cBanco == "033"
			oPrint:Say (0246, 0476,Substr(SE1->E1_NUMBCO,1,Len(Alltrim(SE1->E1_NUMBCO))-1) + " " + Right(AllTrim(SE1->E1_NUMBCO),1),oFont12 )
		EndIf
		...
		If _cBanco == "033"
			oPrint:Say (0271, 0186,"R$",oFont12N )
		EndIf
		...
		oPrint:Say (0271, 0476,AllTrim(Transform(Iif(SE1->E1_PREFIXO <> "RPS",SE1->E1_SALDO + SE1->E1_ACRESC,(SE1->E1_SALDO + SE1->E1_ACRESC) - (aValImps[5] + aValImps[3] + aValImps[4] + aValImps[7] + aValImps[2] + aValImps[6])),"@E 999,999,999.99")),oFont12N )
		...
		If AllTrim(SA1->A1_CGC) $ "11272246000481/10756910000233/52321961000100/"
			oPrint:Say(0316,0026,"CASO OPTE POR REALIZAR ANTECIPAวรO DO PAGAMENTO CONCEDER DESCONTO", oFont10N,100)
			oPrint:Say(0326,0026,"DE 3,75% AO MสS CAPITALIZADO MENSALMENTE.", oFont10N,100)
			oPrint:Say(0336,0026,"DESCONTO DE 49,92% PARA 330 DIAS DE ANTECIPAวรO.", oFont10N,100)
		Else
			oPrint:Say  (0316,0026,"Ap๓s vencimento cobrar mora de R$ ..... " + Alltrim(Transform(((Iif(SE1->E1_PREFIXO <> "RPS",SE1->E1_SALDO,SE1->E1_SALDO - (aValImps[5] + aValImps[3] + aValImps[4] + aValImps[7] + aValImps[2] + aValImps[6])) * _nxJuros)/100)/30,"@E 99,999,999.99"))+ " ao dia", oFont10N,100)
			...
			//EndIf
			oPrint:Say  (0336,0026,"Protestar ap๓s " + Alltrim(_cProtesto) + " dias corridos do vencimento.", oFont10N,100)
		EndIf
		...
		oPrint:Say (0516, 0476,Substr( DtoS(SE1->E1_VENCTO),7,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),5,2 ) + "/" + Substr( DtoS(SE1->E1_VENCTO),1,4 ),oFont12 )
		BuzzBox  (0525,0025,0550,0425)	// Beneficiario
		oPrint:Say (0531, 0026,"Beneficiแrio",oFont06N )
		If _cSubcta == "RVL"
			oPrint:Say (0541, 0026,Alltrim(Substr(SEE->EE_XNOMEVL,1,30)),oFont12 )	// "VIDEOLAR INNOVA S/A"
		Else
			//oPrint:Say (0541, 0026,"Replas Ind. e Comercio de Resinas Plasticas e BOPP Ltda",oFont08 )	// Alltrim(SM0->M0_NOMECOM)
			oPrint:Say (0541, 0026,Alltrim(Substr(SM0->M0_NOMECOM,1,30)),oFont12 )
		EndIf
		If _cBanco == "033"
		...
		If _cBanco == "033"
			oPrint:Say (0541,0436,Substr(Alltrim(_cAgencia),1,4)+"/"+Iif(_cSubCta == "RVL",Substr(Alltrim(_cCodEmp),9,7),Substr(Alltrim(_cCodEmp),6,7)),oFont12,100)
		EndIf
		...
		If _cBanco=="033"
			oPrint:Say (0566, 0476,Substr(SE1->E1_NUMBCO,1,Len(Alltrim(SE1->E1_NUMBCO))-1) + " " + Right(AllTrim(SE1->E1_NUMBCO),1),oFont12 )
		EndIf
		...
		If _cBanco == "033"
			oPrint:Say (0591, 0186,"R$",oFont12N )
		EndIf
		...
		oPrint:Say (0591, 0476,AllTrim(Transform(IIf(SE1->E1_PREFIXO <> "RPS",SE1->E1_SALDO + SE1->E1_ACRESC,(SE1->E1_SALDO + SE1->E1_ACRESC)- (aValImps[5] + aValImps[3] + aValImps[4] + aValImps[7] + aValImps[2] + aValImps[6])),"@E 999,999,999.99")),oFont12N )
		...
		If AllTrim(SA1->A1_CGC) $ "11272246000481/10756910000233/52321961000100/"
			oPrint:Say(0626,0026,"CASO OPTE POR REALIZAR ANTECIPAวรO DO PAGAMENTO CONCEDER DESCONTO", oFont10N,100)
			oPrint:Say(0636,0026,"DE 3,75% AO MสS CAPITALIZADO MENSALMENTE.", oFont10N,100)
			oPrint:Say(0646,0026,"DESCONTO DE 49,92% PARA 330 DIAS DE ANTECIPAวรO.", oFont10N,100)
		Else
			...
			//EndIf
			oPrint:Say  (0646,0026,"Protestar ap๓s " + Alltrim(_cProtesto) + " dias corridos do vencimento.", oFont10N,100)
		EndIf
	...
	EndIf
Next
MsgInfo("Foram Salvos " + AllTrim(Str(nCont)) + " Boletos no Diret๓rio C:\Boleto033\")
WinExec( "Explorer.exe C:\Boleto033" )
...
User Function CalcSant(_aBoletos,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
For _nXx:=1 To Len(_aBoletos)
	If Empty(_aBoletos[_nXx][6])
		...
		lRet := U_CodBco033(_aBoletos[_nXx,2],_aBoletos[_nXx,3],_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
		If !lRet
			RestArea(_aArea)
			Return lRet
		EndIf
...
User Function CodBco033(_cNumeIni,cInull,_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
...
Else
	Do Case
		Case _cBanco == "033"
			_cNossoNum	:= Nosso033(_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
	EndCase
EndIf
...
Do Case
	Case _cBanco == "033" // Banco Santander
		...
		cCpo03 := "9" // Fixo "9"
		cCpo04 := Iif(_cSubcta == "RVL",Substr(Alltrim(_cCodEmp),9,4),Substr(Alltrim(_cCodEmp),6,4)) // Codigo Padrao Cedenteo Banco Santander
...
If _cBanco == "033" // Banco Santander
	_nFator := 0
	_nPeso	:= 2
	_nReturn:= 0
...
Static Function Nosso033(_cBanco,_cAgencia,_cConta,_cSubcta,_Tipo,_EmisIni,_EmisFim,_cTitulo)
