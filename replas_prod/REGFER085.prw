...
/*/--------------------------------------------------
{Protheus.doc} REGFER085
Derivado do padrão - GFER085
Relatorio de Despesa de Frete por Transp, UF e Região
@sample
@author Rafael Domingues - Gustavo H. Baptista
@since 24.10.2018 - 16/04/13
@version 2.0 - 1.0
-----------------------------------------------------/*/

User Function REGFER085()
	...
	Private cAliasRel, cAliasTot, cAliasTransp
...
Static Function ReportDef()
	...
	/*ADICIONADO POR RAFAEL DOMINGUES - TSM - 31/10/2018*/
	/*Início - Tratamento para totalizador geral*/
	oSection4 := TRSection():New(oSection1,"Total Geral ",{""},aOrdem)
	oSection4:SetTotalInLine(.F.)
	oSection4:SetHeaderSection(.F.)
	TRCell():New(oSection4,"Total Geral"   ,"","Total Geral "   	,"@!",7,/*lPixel*/,{|| "Total Geral"})
	TRCell():New(oSection4,""   ,"",""  		,"@!",30,/*lPixel*/,)
	TRCell():New(oSection4,"nTot1" 	,,"Qtd. Doctos"				,"@E 99999999999"			, 11,/*lPixel*/,{||nTot1})
	TRCell():New(oSection4,"nTot2" 	,,"Peso Total"				,"@E 99,999,999,999.99999"	, 20,/*lPixel*/,{||nTot2},,,"RIGHT")
	TRCell():New(oSection4,"nTot3" 	,,"Valor Total"				,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||nTot3},,,"RIGHT")
	TRCell():New(oSection4,"nTot4" 	,,"Volume Total"			,"@E 99,999,999,999.99999"	, 20,/*lPixel*/,{||nTot4},,,"RIGHT")
	TRCell():New(oSection4,"nTot5" 	,,"Qtde Total"				,"@E 99,999,999,999.99999"	, 20,/*lPixel*/,{||nTot5},,,"RIGHT")
	TRCell():New(oSection4,"nTot6" 	,,"Despesa Frete"			,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||nTot6},,,"RIGHT")
	TRCell():New(oSection4,"nTot7" 	,,"$ Frete x Peso"			,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||(nTot6 / nTot2)},,,"RIGHT")
	TRCell():New(oSection4,"nTot8" 	,,"% Frete x Valor"			,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||(nTot6 / nTot3) * 100},,,"RIGHT")
	TRCell():New(oSection4,"nTot9" 	,,"$ Frete x Volume"		,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||(nTot6 / nTot4)},,,"RIGHT")
	TRCell():New(oSection4,"nTot10" ,,"$ Frete x Quantidade"	,"@E 99,999,999,999.99"		, 17,/*lPixel*/,{||(nTot6 / nTot5)},,,"RIGHT")
	/*Fim - Tratamento para totalizador geral*/
...
Static Function ReportPrint(oReport)
	...
	While !((cAliasTransp)->( Eof() )	) // Para cada Transportador
		oSection1:Init()
		oSection1:PrintLine()
		oSection1:Finish()
		oSection2:Init()
		...
		If dbSeek((cAliasTransp)->CDTRP)
			While !(cAliasRel)->( Eof() ) .AND. ;
			(cAliasRel)->CDTRP == (cAliasTransp)->CDTRP// Imprimir todas as Regioes
				oSection2:PrintLine()
		...
		dbSetOrder(1)
		if dbSeek((cAliasTransp)->CDTRP)
			While !(cAliasTot)->(Eof()).AND.						;
			(cAliasTot)->CDTRP == (cAliasTransp)->CDTRP// Por Fim imprime o total do transportador (total parcial)
...
Static Function CarregaDados(oReport)
	...
	While !((cAliasRel)->( Eof() )	)
		...
		/*ADICIONADO POR RAFAEL DOMINGUES - TSM - 31/10/2018*/
		/*Início - Tratamento para totalizador geral*/
		nTot1 += (cAliasRel)->QTDDC
		nTot2 += (cAliasRel)->PESO
		nTot3 += (cAliasRel)->VALOR
		nTot4 += (cAliasRel)->VOLUME
		nTot5 += (cAliasRel)->QTDE
		nTot6 += (cAliasRel)->DESPFRETE
		/*Fim - Tratamento para totalizador geral*/
