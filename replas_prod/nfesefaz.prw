...
Static Function NfeItem(aProd		, aICMS			, aICMSST	, aIPI			, aPIS	   		, aPISST	 , aCOFINS		, aCOFINSST		, aISSQN		, aCST			,;
	...
	If	cVeramb >= "4.00" .and. !empty(aLote)
		If  !empty(aLote[1])
			If ValType(aLote[1]) == "A"
				/*
				Customização desenvolvida no dia 05/05/2026 por solicitação do cliente RELIPEL, isso foi necessario devido o controle de rastreabilidade
				do "Grupo I80 - Rastreabilidade de Produto" que esse cliente passou a usar de forma obrigatoria no seu recebimento e no XML.
				Teve que ser feito o tratamento para pegar os lotes da paletização feita nos filmes da Replas através do PE PE01NFESEFAZ e função GetSLote().
				Dener Lemos.
				*/
				For nX := 1 To Len(aLote)
					cString += '<rastro>'
					cString += '<nLote>'+ConvType(aLote[nX][01])+'</nLote>'
					cString += '<qLote>'+ConvType(aLote[nX][02],12,3)+'</qLote>'
					cString += '<dFab>'+ConvType(aLote[nX][03]) +'</dFab>'
					cString += '<dVal>'+ConvType(aLote[nX][04]) +'</dVal>'
					cString += NfeTag('<cAgreg>',ConvType(aLote[nX][05]))
					cString += '</rastro>'
				Next
			Else
				cString += '<rastro>'
